...
const Cart = () => {
  ...
  const { cartProduct } = store(); //access cartProduct without needing to reference the entire store, cartProduct holds the array of items in the cart, store() is calling your Zustand store function
  ...
  useEffect(() => {
    const totals = cartProduct.reduce(
      (sum, product) => {  // sum = accumulator, product = current item in the array
        sum.regular += product?.regularPrice * product?.quantity; // Add to regular price total
        sum.discounted += product?.discountedPrice * product?.quantity; // Add to discounted price total
        return sum; // Return the updated accumulator
      },
      { regular: 0, discounted: 0 } // Initial value of accumulator is the 2nd arg of the reduce function by standard.
    );
    setTotalAmt(totals);
  }, [cartProduct]);
  return (
    <Container>
      ...
      ) : ( //this below is all only available if the cart is empty.
