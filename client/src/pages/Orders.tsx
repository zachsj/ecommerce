  ...
  useEffect(() => {
    const getData = async () => {
      ...
      try {
        ...
        const docSnap = await getDoc(docRef); //Fetches the document from Firestore.
        if (docSnap.exists()) { //Checks if the Firestore document exists (i.e., the user has placed an order before).
          const orderData = docSnap?.data()?.orders; //Retrieves the "orders" field from the document.
          setOrders(orderData); //Updates state with the fetched orders.
        } else {
          console.log("No orders yet!");
        }
      } catch (error) { //If fetching fails (e.g., network issues, Firestore errors), the error is logged.
        console.log("Data fetching error", error);
      } finally {
        setLoading(false); //This happens regardless of success or failure (to stop the loading state).
      }
    };
    getData(); //Calls getData() immediately after defining it.
  }, []); //Runs only once (because of [] in useEffect).
  return (
    <Container>
      {loading ? (
        <Loading />
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-col gap-3">
            <div className="space-y-6 divide-y divide-gray-900/10"> {/* divide-y adds vertical divider btwn children */}
                ...
                return (
                  <Disclosure as="div" key={order?.paymentId} className="pt-6">
                    {({ open }) => (
                      <>
                        <dt>
                          <DisclosureButton className="flex w-full items-center justify-between text-left text-gray-900 cursor-pointer">
